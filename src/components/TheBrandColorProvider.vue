...
const css = computed(
  () => `/* Generated with SeryiBaran unocss_preset_colors_to_css (unpublished) */
...
function updateBackgroundColorization(isColorize: boolean, newOpacity: number) {
  ...
  document.body.setAttribute(
    'style',
    `--my-colors-colorizedBg: color-mix(in oklch, var(--colors-brand-500) ${newOpacity}%, transparent);`,
  )
}

updateBackgroundColorization(
