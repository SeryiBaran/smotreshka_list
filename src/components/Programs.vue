...
const filteredPrograms = computed(() => {
  const currentProgramIndex = programs?.programs?.findIndex(program => isCurrentProgram(program.scheduledFor, currentTime.value)) || 0
    ...
    <template v-for="program in filteredPrograms.programs">
      <template v-if="program">
        <li :key="program.id + props.channelPrograms.channelId" class="flex gap-2 gap-y-0 relative max-md:flex-wrap" :class="{ 'max-w-3xl 2xl:max-w-4xl': !dontLimitWidth }">
          ...
          <span class="grow" />
          <span class="mb-0.5 pl-2 whitespace-nowrap md:self-center"> <span class="md:hidden">|||||</span> <span>{{ `${getTimeTo(program.scheduledFor, true, currentTime)}` }}</span></span>

          <div
            v-if="showProgress && currentProgram && isCurrentProgram(currentProgram.scheduledFor, currentTime)"
            class="bg-brand-900/20 bottom-0 left-0 right-0 absolute dark:bg-brand-100"
          >
            <div class="border-0 border-b-2 border-b-brand-500 border-solid" :style="{ width: `${isCurrentProgram(program.scheduledFor, currentTime) ? currentProgramPercent : 0}%` }" />
          </div>
        </li>
      </template>
    </template>
